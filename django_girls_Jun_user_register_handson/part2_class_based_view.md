...
![](django_girls_Jun_user_handson/assets/part2/1_no_post_new_form.png)

---

### 記事作成 フォーム表示

- 真っ白な画面に対処して、記事を作成するための表示を作成したい
...
HTTPメソッドの説明（TODO：なぜ作れないのか本）

GETの場合（ブラウザでページを表示した場合）
関数ベースビューで`if request.method == "POST":`がFalseの場合と同じ処理をしている

1-2月のハンズオンのテキスト見直してもいいかも
GET：ページ表示
POST：データを送信してWebアプリ側で処理してもらう（結果に応じたページ表示）
...
POSTの場合
関数ベースビューで`if request.method == "POST":`がTrueの場合と同じ処理
...
![記事作成画面の表示にはログインが求められます](django_girls_Jun_user_handson/assets/part2/2_required_login.png)
